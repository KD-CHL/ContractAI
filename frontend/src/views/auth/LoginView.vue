    ...
    <div class="form-panel">
      <div class="form-container">
        <div class="form-header">
          <h2>{{ isBootstrap ? '创建管理员账户' : '欢迎回来' }}</h2>
          <p>{{ isBootstrap ? '首次使用，请创建组织管理员账户' : '登录您的 ContractGuard 账户' }}</p>
        </div>
        ...
        >
          <n-form-item label="组织名称" path="org_name">
            <n-input
              v-model:value="registerForm.org_name"
        ...
        <div v-if="!isBootstrap" class="form-footer">
          <span>还没有账户？</span>
...
onMounted(async () => {
  try {
    ...
    // If bootstrap check fails, show normal login
  }
})
...
async function handleLogin() {
  ...
  try {
    ...
    errorMsg.value =
      err.response?.data?.detail || err.response?.data?.message || '登录失败，请检查邮箱和密码'
  } finally {
    loading.value = false
  }
}
...
async function handleRegister() {
  ...
  try {
    await authStore.register({
      org_name: registerForm.org_name,
