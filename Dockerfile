...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    CONTRACT_GUARD_AUTH_COOKIE_SECURE=true \
    CONTRACT_GUARD_CORS_ORIGINS=https://contractai-ruddy.vercel.app \
...
RUN mkdir -p /data \
    && chown -R contractguard:contractguard /app /data
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 \
  CMD python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:7860/live', timeout=2)" || exit 1
